...
def mri_to_png(mri_file, png_file):
    ...
    print(mri_file.name)
    print('haha')
    image = sitk.ReadImage(mri_file.name)
    image_array = sitk.GetArrayFromImage(image)
    shape = image_array.shape

    #Convert to float to avoid overflow or underflow losses.
    image_2d = image_array.astype(float)
        ...
        for mri_file in os.listdir(mri_sub_folder):
            ...
            if os.path.isfile(mri_file_path):
                ...
                try:
                    ...
                    os.system('rm -rf %s' % png_folder)


# convert_folder(sys.argv[1], sys.argv[2])
for i in range(1, 10):
    convert_folder('./data/sub00%d/MRI/T2' % i, './output/sub00%d' % i)
for i in range(11, 41):
    convert_folder('./data/sub0%d/MRI/T2' % i, './output/sub0%d' % i)
